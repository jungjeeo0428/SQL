...
-- MySQL에서 외래 키 제약 조건 비활성화 (순서와 관계없이 DROP/CREATE를 위함)
SET FOREIGN_KEY_CHECKS = 0;

-- 기존 테이블 삭제 (DROP TABLE IF EXISTS 사용)
...
-- ===================================
-- 1. lprod 테이블 (Large Product Category) 생성
-- ===================================
CREATE TABLE lprod
(
  lprod_id  INT          NOT NULL,
  lprod_gu  CHAR(4)      NOT NULL,
  lprod_nm  VARCHAR(40)  NOT NULL,
...
-- ===================================
-- 2. buyer 테이블 (Supplier/Buyer) 생성
-- ===================================
CREATE TABLE buyer
(  buyer_id           CHAR(6)       NOT NULL,   -- 거래처코드 (Buyer ID)
   buyer_name         VARCHAR(50)   NOT NULL,   -- 거래처명 (Buyer Name)
   buyer_lgu          CHAR(4)       NOT NULL,   -- 취급상품대분류 (Product Category)
   buyer_bank         VARCHAR(40),              -- 은행 (Bank)
   buyer_bankno       VARCHAR(40),              -- 계좌번호 (Account No)
   buyer_bankname     VARCHAR(15),              -- 예금주 (Account Holder)
   buyer_zip          CHAR(7),                  -- 우편번호 (Zip Code)
   buyer_add1         VARCHAR(100),             -- 주소1 (Address 1)
   buyer_add2         VARCHAR(80),              -- 주소2 (Address 2)
   buyer_comtel       VARCHAR(14)   NOT NULL,   -- 전화번호 (Company Tel) 
   buyer_fax          VARCHAR(20)   NOT NULL,   -- fax번호 (Fax No)
   buyer_mail         VARCHAR(40)   NOT NULL,   -- E-mail주소 (E-mail)
   buyer_charger      VARCHAR(10),              -- 담당자 (Charger)
   buyer_telext       VARCHAR(2)
);

-- ALTER TABLE 구문 통합 (Schema modification)
ALTER TABLE buyer 
   MODIFY COLUMN buyer_name VARCHAR(40) NOT NULL,
   ADD CONSTRAINT pk_buyer PRIMARY KEY(buyer_id),
   ADD CONSTRAINT fr_buyer_lgu FOREIGN KEY(buyer_lgu) 
...
-- ===================================
-- 3. prod 테이블 (Product) 생성
-- ===================================
CREATE TABLE  prod
(  prod_id             VARCHAR(10)      NOT NULL,     -- 상품코드 (Product ID)
   prod_name           VARCHAR(40)      NOT NULL,     -- 상품명 (Product Name)
   prod_lgu            CHAR(4 )         NOT NULL,     -- 상품분류 (Product Category)
   prod_buyer          CHAR(6)          NOT NULL,     -- 공급업체(코드) (Supplier ID)
   prod_cost           INT              NOT NULL,     -- 매입가 (Cost Price)
   prod_price          INT              NOT NULL,     -- 소비자가 (Consumer Price)
   prod_sale           INT              NOT NULL,     -- 판매가 (Sale Price)
   prod_outline        VARCHAR(100)     NOT NULL,     -- 상품개략설명 (Outline)
   prod_detail         TEXT,                           -- 상품상세설명 (Detail) - CLOB -> TEXT
   prod_img            VARCHAR(40)      NOT NULL,     -- 이미지(소) (Image)
   prod_totalstock     INT              NOT NULL,     -- 재고수량 (Total Stock)
   prod_insdate        DATE,                           -- 신규일자(등록일) (Insert Date)
   prod_properstock    INT              NOT NULL,     -- 안전재고수량 (Proper Stock)
   prod_size           VARCHAR(20),                    -- 크기 (Size)
   prod_color          VARCHAR(20),                    -- 색상 (Color)
   prod_delivery       VARCHAR(255),                   -- 배달특기사항 (Delivery Notes)
   prod_unit           VARCHAR(6),                     -- 단위(수량) (Unit)
   prod_qtyin          INT,                            -- 총입고수량 (Total Quantity In)
   prod_qtysale        INT,                            -- 총판매수량 (Total Quantity Sale)
   prod_mileage        INT,                            -- 개당 마일리지 점수 (Mileage)
...
-- ===================================
-- 4. buyprod 테이블 (Purchase/Stock-in Product) 생성
-- ===================================
CREATE TABLE  buyprod
(  buy_date  DATE           NOT NULL,             -- 입고일자 (Purchase Date)
   buy_prod  VARCHAR(10)    NOT NULL,             -- 상품코드 (Product ID)
   buy_qty   INT            NOT NULL,             -- 매입수량 (Quantity)
   buy_cost  INT            NOT NULL,             -- 매입단가 (Cost Price)
...
-- ===================================
-- 5. member 테이블 (Member) 생성
-- ===================================
CREATE TABLE  member
(  mem_id                VARCHAR(15)    NOT NULL,   -- 회원ID (Member ID)
   mem_pass              VARCHAR(15)    NOT NULL,   -- 비밀번호 (Password)
   mem_name              VARCHAR(20)    NOT NULL,   -- 성명 (Name)
   mem_regno1            CHAR(6)        NOT NULL,   -- 주민등록번호앞6자리 (RRN Part 1)
   mem_regno2            CHAR(7)        NOT NULL,   -- 주민등록번호뒤7자리 (RRN Part 2)
   mem_bir               DATE,                      -- 생일 (Birthday)
   mem_zip               CHAR(7)        NOT NULL,   -- 우편번호 (Zip Code)
   mem_add1              VARCHAR(100)   NOT NULL,   -- 주소1 (Address 1)
   mem_add2              VARCHAR(80)    NOT NULL,   -- 주소2 (Address 2)
   mem_hometel           VARCHAR(14)    NOT NULL,   -- 집전화번호 (Home Tel)                                
   mem_comtel            VARCHAR(14)    NOT NULL,   -- 회사전화번호 (Company Tel)                              
   mem_hp                VARCHAR(15),              -- 이동전화 (Mobile Phone)
   mem_mail              VARCHAR(40)    NOT NULL,   -- E-mail주소 (E-mail)
   mem_job               VARCHAR(40),              -- 직업 (Job)
   mem_like              VARCHAR(40),              -- 취미 (Hobby)
   mem_memorial          VARCHAR(40),              -- 기념일명 (Memorial Name)
   mem_memorialday       DATE,                      -- 기념일날짜 (Memorial Day)
   mem_mileage           INT,                       -- 마일리지 (Mileage) - NUMBER(10) -> INT
   mem_delete            CHAR(1),                   -- 삭제여부 (Delete Status)
...
-- ===================================
-- 6. cart 테이블 (Shopping Cart/Order Details) 생성
-- ===================================
CREATE TABLE  cart
(
   cart_member      VARCHAR(15)    NOT NULL,       -- 회원ID (Member ID)
   cart_no          CHAR(13)        NOT NULL,       -- 주문번호 (Order No)
   cart_prod        VARCHAR(10)    NOT NULL,       -- 상품코드 (Product ID)
   cart_qty         INT             NOT NULL,       -- 수량 (Quantity)
...
select mem_name
  , mem_mileage
  , case when mem_mileage > 8000 then 'VIP'
         when mem_mileage > 5000 then 'GOLD'
    else 'SILVER'
    end as '등급'
from member
order by 3 desc;
...
select mem_name
	, substring(mem_regno2,1,1) 
    , case when substring(mem_regno2,1,1) = 1 then 'M'
      else 'F'
    end 'gender'
from member;

select dayname(mem_memorialday), count(dayname(mem_memorialday))
from member
group by dayname(mem_memorialday);

show tables;
select *
from prod;
select cart_member, count(distinct cart_no)
from cart
...
select member.mem_name, prod.prod_name, prod.prod_sale
from member
inner join cart
on member.mem_id = cart.cart_member
inner join prod
...
where member.mem_id = 'a001'
group by member.mem_name, prod.prod_name, prod.prod_sale;
...
(select dept_nm
from departmemnts d
where e.dept_id = d.dept_id) as dept_nm
from emplyees e;
...
select*
from prod;
-- 총구매 합산 금액이 큰 회원 10명 출력
-- member.mem_id, member.mem_name, 
-- cart_member = member.mem_id cart_no, cart_prod, cart_qty
-- 금액을 모름으로
-- cart.cart_prod = prod.prod_id
-- sum(prod.prod_sale)

select*
from (select member.mem_id, member.mem_name, sum(prod_sale) as 총구매금액
		from member
        inner join cart
        on cart_member = member.mem_id
        inner join prod
        on cart.cart_prod = prod.prod_id
        group by  member.mem_id, member.mem_name
        order by sum(prod_sale) desc
        limit 10
        )a;
